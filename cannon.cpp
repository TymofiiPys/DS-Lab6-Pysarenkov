...
void RandomDataInitialization(int **pAMatrix, int **pBMatrix,
                              int Size)
{
    ...
    for (i = 0; i < Size; i++)
        for (j = 0; j < Size; j++)
        {
            pAMatrix[i][j] = (int)rand() / double(1000);
            pBMatrix[i][j] = (int)rand() / double(1000);
        }
    printf("%d", i);
}
...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        RandomDataInitialization(A, B, rows);
        // // Read matrix A
        // for (int i = 0; i < rows; i++) {
